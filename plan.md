...
app/DI scaffolding, settings, and the `POST /api/resume/upload` endpoint with file
validation and disk storage.

Steps below are in implementation order.

---

## 1. Finish CV extraction (stubs exist)

- `DocumentParser.extract_text` — parse PDF via `pypdf`, DOCX via `python-docx`.
  Text is transient: feeds extraction, never persisted.
- `LLMService.extract_resume` — one Claude Haiku call, raw text → resume JSON
  (`is_valid_resume`, name, roles, skills, total_years, education, certs).
  `is_valid_resume: false` → short-circuit with a friendly error.
- `GET /api/resume` — list previously uploaded CVs so a resume can be reused
  with zero LLM calls.
...
- **Job extraction** — Haiku call per job, free text → job JSON
...
- **Narrative** — one Sonnet call per job: CV JSON + job JSON + computed diff + score →
